...
import { canFormatCellDetailJson, cellDetailEditorText, compactJsonText, defaultCellDetailTab, formatJsonText, isGeometryColumnType, linkedCellDetailTarget, valueEditorActions, visibleCellDetailTabs, type CellDetailTab } from "@/lib/cellDetailPresentation";
...
const canCompactDetailJson = computed(() => {
  const detail = activeCellDetail.value;
  if (!detail || !isEditingDetail.value || !canFormatCellDetailJson(detailEditValue.value, detail.type)) return false;
  return compactJsonText(detailEditValue.value) !== detailEditValue.value.trim();
});
const showCompactDetailJson = computed(() => {
  return canCompactDetailJson.value;
});
                ...
                <div data-native-clipboard class="flex-1 min-h-0 overflow-auto px-3 pt-3 text-xs" :class="[sideDetailValueFillsHeight ? 'flex flex-col gap-3' : 'space-y-3', isEditingDetail && !cellDetailPanelIsBottom ? 'pb-1' : 'pb-3']">
                  ...
                  <div class="space-y-1" :class="[{ 'min-h-0 flex flex-col': sideDetailValueFillsHeight }, sideDetailValueFillsHeight && !(activeCellDetail.imagePreviewUrl && !isEditingDetail) ? 'flex-1' : '', activeCellDetail.imagePreviewUrl && !isEditingDetail ? 'shrink-0' : '']">
                    <div class="flex min-h-5 items-center justify-between gap-2">
                      ...
                      <div class="flex items-center gap-1">
                        <Button v-if="showCompactDetailJson" variant="ghost" size="sm" class="h-5 gap-1 px-1.5 text-xs" :title="t('grid.compactJson')" @click="compactDetailJson">
                          <Code2 class="h-3 w-3" />
                          {{ t("grid.compactJson") }}
                        </Button>
